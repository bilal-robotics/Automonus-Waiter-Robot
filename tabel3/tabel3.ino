...
int finalDestination = 6;   // Mission 1 has 6 junctions
int patternMode = 1;        // 1 = Mission 1, 2 = Mission 2
...
void loop() {
 ...
 if (left == 1 && center == 1 && right == 1 && leftTurnCounter == finalDestination) {
    ...
    if (patternMode == 1) {
      ...
      patternMode = 2;
      leftTurnCounter = 0;
      finalDestination = 2;   // Mission 2 has 2 junctions only
      Serial.println("Switching to Mission 2...");
  ...
  if (left == 1 && center == 1 && leftTurnCounter < finalDestination) {
    ...
    if (patternMode == 1) {
      // Mission 1: Turn at 1,4,5,6 ; Skip 2,3
      if (leftTurnCounter == 1 || leftTurnCounter == 4 || 
          leftTurnCounter == 5 || leftTurnCounter == 6) {
        setMotor(70, 70); delay(150);
        setMotor(-70, 70); delay(250);   // Left turn
      } else if (leftTurnCounter == 2 || leftTurnCounter == 3) {
        setMotor(baseSpeed, baseSpeed); delay(150); // Skip = straight
      }
    } 
    else if (patternMode == 2) {
  // Mission 2: Only Turn at 1
